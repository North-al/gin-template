...
type User struct {
	...
	Birthday  *format.LocalTime `gorm:"comment:生日"`
	Status    int8              `gorm:"default:1;comment:状态 1:正常 0:禁用"`
	CreatedAt format.LocalTime  `gorm:"autoCreateTime;comment:账户创建时间"`
	UpdatedAt format.LocalTime  `gorm:"autoUpdateTime;comment:账户更新时间"`
